...
// A number that counts up when it enters the viewport. Writes straight to
// textContent (no re-render per frame), rises through the pentatonic ticks
// when sound is on, and renders the final value immediately under reduced
// motion. The wrapper carries the finished value for assistive tech.
export function CountUp({ value, decimals = 1, duration = 1.6, delay = 0, className, ticks = true }) {
  ...
  useEffect(() => {
    ...
    if (!inView) { el.textContent = fmt(0, decimals); return undefined; }
    if (prefersReducedMotion()) { el.textContent = fmt(value, decimals); return undefined; }
    const controls = animate(0, value, {
      ...
      onComplete: () => { el.textContent = fmt(value, decimals); },
    });
    return () => controls.stop();
  }, [inView, value, decimals, duration, delay, ticks]);

  return (
    <span className={className} aria-label={fmt(value, decimals)}>
